...
def _person_provider_config(config: AppConfig) -> tuple[str, dict[str, object]]:
    enabled = [
        ("yolo11n", config.providers.person_yolo),
        ("rtdetr-l", config.providers.person_rtdetr),
    ]
...
def generate_automatic_signals(
    video_path: Path, config: AppConfig
) -> tuple[list[Signal], list[TrackObservation], tuple[int, int] | None]:
    """Generate review candidates. Provider failures are explicit unless optional."""
    ...
    anomaly = None
    if anomaly_raw["enabled"]:
        anomaly = AnomalyProvider(_settings(AnomalySettings, anomaly_raw))
    cover = None
    if cover_raw["enabled"]:
        cover = CoverHeuristicProvider(_settings(CoverSettings, cover_raw))
    movement = None
    if movement_raw["enabled"]:
        movement = CameraMovementProvider(_settings(MovementSettings, movement_raw))
    ...
    try:
        if person_name == "rtdetr-l":
            settings = _settings(PersonRtdetrSettings, person_raw)
            person = RtdetrPersonProvider(config.providers.rtdetr_l_weights, settings)
        else:
            settings = _settings(PersonYoloSettings, person_raw)
            person = YoloPersonProvider(config.providers.yolo11n_weights, settings)
        person._load_model()
